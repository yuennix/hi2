...
import type { MailMessage } from "@/hooks/use-mail-websocket";
import { ArrowLeft, Clock, User, Download, FileText, AlertTriangle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { format } from "date-fns";

function loadMessageFromStorage(email: string, messageId: string): MailMessage | null {
  try {
    const raw = localStorage.getItem(`hi2_messages_${email}`);
    if (!raw) return null;
...
export default function MessageDetail() {
  ...
  useEffect(() => {
    ...
    // Messages are stored locally via the WebSocket hook — read from localStorage
    const msg = loadMessageFromStorage(email, messageId);
    setMessage(msg);
    setIsLoading(false);
  }, [messageId, email]);
      ...
      {isLoading ? (
        <div className="border border-primary/20 bg-card p-6 space-y-6">
          ...
          <div className="space-y-3">
            {[1, 2, 3, 4].map((i) => (
              <Skeleton key={i} className="h-4 w-full bg-primary/20" />
            ))}
          </div>
        </div>
      ) : !message ? (
        <div className="border border-destructive/50 p-8 text-center bg-destructive/5">
          ...
          <h3 className="text-destructive font-bold">MESSAGE_NOT_FOUND</h3>
          <p className="text-destructive/70 text-sm mt-1">
            This message may have been cleared or expired.
          </p>
          <Button
            variant="outline"
            className="border-primary text-primary rounded-none mt-4"
            onClick={() => setLocation("/")}
          >
            <ArrowLeft className="w-4 h-4 mr-2" /> RETURN
          </Button>
        ...
        <div className="border border-primary/30 bg-card shadow-xl shadow-primary/5 relative overflow-hidden">
          ...
          <div className="border-b border-primary/20 p-6 bg-primary/5 space-y-4">
            <h1
              className="text-xl md:text-2xl font-bold text-foreground break-words"
            ...
            <div className="flex flex-col md:flex-row gap-4 text-sm text-muted-foreground">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-primary/80" data-testid="text-message-from">
                  {message.fromName
              ...
              <div className="flex items-center gap-2">
                ...
                <span data-testid="text-message-date">
                  {format(new Date(message.receivedAt), "MMM d, yyyy HH:mm:ss")}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="border-b border-primary/20 p-4 bg-background">
              ...
              <div className="flex flex-wrap gap-2">
                {message.attachments.map((att, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 px-3 py-1.5 border border-primary/30 bg-primary/5 text-xs"
                  >
          ...
          {/* Body */}
          <div
            className="p-6 min-h-[300px] bg-white text-black font-sans"
            data-testid="div-message-body"
          >
            {message.bodyHtml ? (
              <div
